...
impl RiceEncoderOptions {
    ...
    fn best_partition_and_params(&self, residuals: &Vec <i64>) -> (Vec <u8>, u8) {
        ...
        for order in 0..Self::max_rice_partition_order(self.num_samples){
            let mut best_parameters:(Vec <u8>, u64) = Self::best_parameters(self, order, &residuals).unwrap_or((vec![], 0));

            let n_partitions:u64 = self.num_samples << best_parameters.1;
            let n_partition_samples = best_parameters.1;

            let mut num_bits = 0;
            for partition in 0..n_partitions{
                let mut partition_samples:Vec<i64> = Vec::new();
                for r in 0..n_partition_samples{
                    partition_samples.push(residuals[partition as usize + r as usize])
                }
    ...
    fn best_parameters(&self, partition_order: u8, residuals: &Vec <i64>) -> Option <(Vec <u8>, u64)> {
        ...
        let n_partitions:u64 = self.num_samples << 2u8.pow(partition_order as u32);
        ...
        for partition in 0..n_partitions{
            ...
            best_param = ((abs_residual_sum - 1).ilog2() - (n_partition_samples).ilog2() + 1) as u8;
    ...
    pub fn encode_by_partition(&self, residuals: &Vec <i64>)  -> (Vec <RiceEncodedStream>, u8) {
        ...
        let n_partitions:u64 = self.num_samples << 2u8.pow(order as u32);
...
fn main(){
    let rice = RiceEncoderOptions::new(5,2);

    rice.best_parameters(6, &vec![23 -1, -2, 4, 5]);
}
